...
//    private static final String UPDATE_ROW_STATEMENT = "UPDATE %s SET %s WHERE id = '%s';";
    ...
    public StateObjectSql(PostgresServer pgServer) {
        undoLog = new TreeMap<>();
        this.pgServer = pgServer;
        session = initSessionPostgres();
        initDb();
    ...
    private void rollbackVersionFromTable(long version, String table) {
        String selectVersionRowsStatement = String.format(SELECT_VERSION_RECORDS_FORMAT, table, version);
        Command rollbackOperationCommand = session.prepareLocal(selectVersionRowsStatement);
        ResultInterface rowsToRollback = rollbackOperationCommand.executeQuery(0, false);
        while (rowsToRollback.next()) {
    ...
    private void deleteRecord(String id, String tableName) {
        String sqlStatement = String.format(DELETE_ROW_FORMAT, tableName, id);
        Command command = session.prepareLocal(sqlStatement);
        command.executeUpdate(null);
    }

    private void setLatestRowValue(ResultInterface result, String tableName) {
        int columnsCount = result.getVisibleColumnCount();
        Value[] row = result.currentRow();
        StringJoiner updateRowJoiner = new StringJoiner(", ");
        for (int i = 3; i < columnsCount; i++) {    // first 3 cols are technical history cols, then original record
            String colName = result.getColumnName(i);
            String value = row[i].toString();
            updateRowJoiner.add(colName + "=" + value);
        }
        String updateStatement = String.format(UPDATE_ROW_STATEMENT, tableName, updateRowJoiner, row[3].toString());
        Command command = session.prepareLocal(updateStatement);
        command.executeUpdate(null);
    }
    ...
    /*
        SQL Logic below
     */
    private SessionLocal initSessionPostgres() {
    ...
    private SessionLocal initSessionMySQL() {
        Properties info = new Properties();
        info.put("MODE", "MySQL");
        info.put("DATABASE_TO_LOWER", "TRUE");
        info.put("DEFAULT_NULL_ORDERING", "HIGH");
