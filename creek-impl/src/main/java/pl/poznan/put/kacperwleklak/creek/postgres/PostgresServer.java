 ...
 *
 * Copied from https://github.com/h2database/h2database
 *
 * @author Thomas Mueller
 * @author Sergi Vladykin 2009-07-03 (convertType)
 */
@Slf4j
public class PostgresServer implements Service {
    ...
    private int port = PostgresServer.DEFAULT_PORT;
    private boolean portIsSet;
    private boolean stop;
    private ServerSocket serverSocket;
    private final Set<PostgresClientThread> running = Collections.
            synchronizedSet(new HashSet<PostgresClientThread>());
    ...
    public PostgresServer(OperationExecutor operationExecutor) {
        ...
        try {
            while (!stop) {
                ...
                if (!allow(s)) {
                    ...
                    PostgresClientThread c = new PostgresClientThread(s, this, operationExecutor);
                    running.add(c);
                    int id = pid.incrementAndGet();
                    c.setProcessId(id);
                    Thread thread = new Thread(c, threadName + " pg-thread-" + id);
    ...
    public void stop() {
        ...
        for (PostgresClientThread c : new ArrayList<>(running)) {
            c.close();
            try {
     ...
     * @return the thread
     */
    PostgresClientThread getThread(int processId) {
        for (PostgresClientThread c : new ArrayList<>(running)) {
