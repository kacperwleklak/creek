...
def run_node(host: str, port: int, pg_port: int, server_port: int, node_id: int, nodes: list[str],
    ...
    with open(log_file_name, "w") as log_file:
        return subprocess.Popen(["C:\\Program Files\\graalvm\\bin\\java.exe",
                                ...
                                env=my_env, creationflags=subprocess.CREATE_NEW_CONSOLE)
...
def run_benchmarks(benchmarks: int, operations_cnt: int, cab_prob: float) -> list[subprocess.Popen]:
    ...
    node_info_file_pattern = ".\\props\\creek{node_id}.properties"
    return [run_ycsb(node_props_file=node_info_file_pattern.format(node_id=node_id),
                     workload_file=workload_file_pattern.format(ops_cnt=operations_cnt),
                     ops_count=operations_cnt,
                     node_id=node_id,
                     cab_prob=cab_prob,
                     total_nodes=benchmarks)
            for node_id in range(1, benchmarks + 1)]
...
if __name__ == '__main__':
    progress_bar = tqdm(total=4 * len(OPERATIONS_COUNTS) * len(CAB_PROBABILITIES))
    started_nodes = []
    started_benchmarks = []
